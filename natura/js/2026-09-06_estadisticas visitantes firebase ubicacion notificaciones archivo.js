...
import { initializeApp, getApps } from "https://www.gstatic.com/firebasejs/10.12.5/firebase-app.js";
        ...
        try {
          await runTransaction(visitaRef, actual => {
            ...
            return {
              ...
              direccion: normalizarDireccion(ubicacion.direccion || ""),
              lat: numeroUbicacion(ubicacion.lat),
              lng: numeroUbicacion(ubicacion.lng),
              acc: numeroUbicacion(ubicacion.acc),
              maps_url: construirMapsUrl(ubicacion.lat, ubicacion.lng, ubicacion.ciudad, ubicacion.pais),
    ...
    async function obtenerCiudadDesdeGps(coords) {
      ...
      const region = normalizarTexto(datos.principalSubdivision || "");
      const codigoPostal = normalizarTexto(datos.postcode || "");
      ...
      const sitio = extraerSitioInformativo(datos.localityInfo?.informative, ciudad, codigoPostal);
      const direccion = construirDireccionLegible(sitio, ciudad, region, codigoPostal, pais);
        ...
        try {
          ...
          if (ciudad || pais) {
            return {
              ...
              pais,
              direccion: construirDireccionLegible("", ciudad, "", "", pais),
              fuente: "IP",
              lat: "",
              lng: "",
              acc: ""
    ...
    function extraerSitioInformativo(items, ciudad, codigoPostal) {
      if (!Array.isArray(items)) return "";
      const ciudadNorm = normalizarTexto(ciudad).toLocaleLowerCase("es");
      const postalNorm = normalizarTexto(codigoPostal).toLocaleLowerCase("es");
      const candidatos = items
        .map(item => normalizarTexto(item?.name || ""))
        .filter(Boolean)
        .filter(nombre => {
          const valor = nombre.toLocaleLowerCase("es");
          return valor !== ciudadNorm && valor !== postalNorm && !/^\d{4,}$/.test(valor);
        });
      return candidatos.at(-1) || "";
    }

    function construirDireccionLegible(sitio, ciudad, region, codigoPostal, pais) {
      const vistos = new Set();
      const partes = [sitio, ciudad, region, codigoPostal, pais]
        .map(normalizarTexto)
        .filter(Boolean)
        .filter(valor => {
      ...
      return normalizarDireccion(partes.join(", "));
    }

    function construirMapsUrl(lat, lng, ciudad, pais) {
      const latNum = Number(lat);
      const lngNum = Number(lng);
      if (Number.isFinite(latNum) && Number.isFinite(lngNum)) {
        return `https://www.google.com/maps/search/?api=1&query=${latNum},${lngNum}`;
      }
      const consulta = [ciudad, pais].map(normalizarTexto).filter(Boolean).join(", ");
      return consulta
        ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(consulta)}`
        : "";
    }

    function numeroUbicacion(valor) {
